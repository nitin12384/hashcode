...
//global var
int nPhotos=0; // <= 10,0000 
int nHorPhotos=0;
int nSlides=0;
int totScore = 0 ;

bool dM =  0      ;		// debug mode
...
void optimizeScore1(Slide *slideshow, Photo* photos)
{
	...
	for(int i = 0; i<nSlides-1; i++ )
	{
		...
		{	
			...
			scoreNew = ( slideshow + i)->interestF( slideshow + j, photos) ;
...
void processFile(const char * inpName = "def.in" , const char * outName = "def.out") 
{
	...
	if(sM) cout << "Input was succesful\n" << "File has nPhotos = " << nPhotos << " and Hor Photos = " << nHorPhotos << endl ;
	...
	if(sM) cout << "Slideshow has score = " << calcScore(slideshow, photos) << endl ;		// Redundant ********
	
	//optimize via sort
	optimizeScore1(slideshow, photos) ;
	int scoreNow = calcScore(slideshow, photos) ;
	
	if(sM) cout << "Now Slideshow has score = " << scoreNow << endl ;
	...
	for(int i=0; i<nTags1; i++) // all tags in slide1 and photo1
	{
		...
		for(int j=0; j< (slide2->nTags1) ; j++) // all tags of photo1 of slide2
		{
		 	if( search == ( (photos + slide2->photo1)->tags[j] ) )
		 	{
		 		...
		 		if(dM) cout << "A Common Tag was found in photo1 !! \n" ;
		 		nCommonTags++ ;
		 		found = 1;
		 		break;
		 	}
		...
		if(!(slide2->isHor) && (!found)) // slide 2 is vertical its got more tags and we didn't found our search till yet
		{
			for(int j=0; j< (slide2->nTags2) ; j++) // all tags of photo2 of slide2
			{
			 	...
			 	{
			 		// found a tag of s1 in s2
		 			if(dM) cout << "A Common Tag was found in photo 2!! \n" ;
			 		nCommonTags++ ;
			 		found = 1;
			 		break;
			 	}
...
int main()
{
	...
	processFile("./qualification_round_2019.in/a_example.txt", "a.out") ; // 1 KB
	
	processFile("./qualification_round_2019.in/c_memorable_moments.txt", "c.out") ; // 47 KB
	
	
//	processFile("./qualification_round_2019.in/d_pet_pictures.txt", "d.out") ; // 3866 KB
	
//	processFile("./qualification_round_2019.in/e_shiny_selfies.txt", "e.out") ; // 6267 KB
	
	processFile("./qualification_round_2019.in/b_lovely_landscapes.txt", "b.out") ; // 8828 KB
